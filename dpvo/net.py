...
class Update(nn.Module):
    ...
    def forward(self, net, inp, corr, flow, ii, jj, kk):
        ...
        net = net + inp + self.corr(corr) #inp: the patch `ctx` feature; corr: the patch `cat` feature
        net = self.norm(net)
        ...
        net = net + self.agg_ij(net, ii*12345 + jj) # different patches, but from the same frame and to the same destination frame; 12345 is a random number
