...
from .optim_utils import SE3_to_Sim3, make_pypose_Sim3, ransac_umeyama, run_DPVO_PGO, run_DPVO_PGO_sychronize
from .retrieval import ImageCache, RetrievalDBOW, RetrievalNetVLAD


class LongTermLoopClosure:

    def __init__(self, cfg, patchgraph, nvlad_db):
        self.cfg = cfg

        # Data structures to manage retrieval
        self.retrieval = RetrievalNetVLAD(nvlad_db)
    ...
    def __call__(self, img, n, tstamp):
        img_np = K.tensor_to_image(img)
        self.retrieval(img_np, n, tstamp)
    ...
    def estimate_3d_keypoints(self, i):
        ...
        fastba.BA(poses, patches, intrinsics,
            target, weight, lmbda, ii, jj, kk, 3, 3, iterations=6)
    ...
    def close_loop(self, i, j, n):
        ...
        output = run_DPVO_PGO_sychronize(pred_poses.data, loop_poses.data, loop_ii, loop_jj)
