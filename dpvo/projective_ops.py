...
def transform(poses, patches, intrinsics, ii, jj, kk, depth=False, valid=False, jacobian=False, tonly=False):
    ...
    # transform, note that the pose is w2c
    Gij = poses[:, jj] * poses[:, ii].inv() # points in the i-th coordinate to j-th coordinate
    ...
    # project
    x1 = proj(X1, intrinsics[:,jj], depth) # x1: [1, N, p, p, 2], the last dimension is the projected 2d coordinates
