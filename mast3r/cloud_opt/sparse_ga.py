...
from mast3r.debug import draw_two_point_clouds
...
def sparse_global_alignment(imgs, pairs_in, cache_path, model, subsample=8, desc_conf='desc_conf',
    ...
    mst = compute_min_spanning_tree(pairwise_scores) # pairwise_scores is in fact the point/feature prediction scores
...
def sparse_scene_optimizer(imgs, subsample, imsizes, pps, base_focals, core_depth, anchors, corres, corres2d,
    ...
    quats = [nn.Parameter(vec0001.clone()) for _ in range(len(imgs))] # initial guess of all camera transformations
    ...
    for img in imgs:
        ...
        if K is not None:
            ...
            pps[idx] = pp
        pps[idx] /= imsizes[idx]  # default principal_point would be (0.5, 0.5); NOTE it seems principal points are also optimized?
    ...
    pps = [nn.Parameter(pp.to(dtype)) for pp in pps] # pps are also optimizable?
    diags = imsizes.float().norm(dim=1)
    min_focals = 0.25 * diags  # diag = 1.2~1.4*max(W,H) => beta >= 1/(2*1.2*tan(fov/2)) ~= 0.26
    max_focals = 10 * diags
    log_focals = [nn.Parameter(f.view(1).log().to(dtype)) for f in base_focals] # `base_focals` are estimated from canonical views.
    assert len(mst[1]) == len(pps) - 1

    def make_K_cam_depth(log_focals, pps, trans, quats, log_sizes, core_depth):
        # make intrinsics
        focals = torch.cat(log_focals).exp().clip(min=min_focals, max=max_focals) # NOTE Need to clip focals?
        pps = torch.stack(pps)
        ...
        # security! optimization is always trying to crush the scale down # Question: Is there no scale regularization? So, it seems size is the scale factor.
        sizes = torch.cat(log_sizes).exp()
        global_scaling = 1 / sizes.min()

        # compute distance of camera to focal plane
        # tan(fov) = W/2 / focal
        z_cameras = sizes * median_depths * focals / base_focals # median_depths is per-frame optimization; Question: what is z_cameras?
        ...
        tmp_cam2w[mst[0]] = rel_cam2cam[mst[0]] # mst: the minimum spanning tree
        for i, j in mst[1]: # edges in the mst
            # i is the cam_i_to_world reference, j is the relative pose = cam_j_to_cam_i
            tmp_cam2w[j] = tmp_cam2w[i] @ rel_cam2cam[j] # cam2word @ cam2cam (relative pose)? Question
        tmp_cam2w = torch.stack(tmp_cam2w)

        # smart reparameterizaton of cameras
        trans_offset = z_cameras.unsqueeze(1) * torch.cat((imsizes / focals.unsqueeze(1) * (0.5 - pps), ones), dim=-1) # last dimension is 1, shape: (n_images, 3)
        new_trans = global_scaling * (tmp_cam2w[:, :3, 3:4] - tmp_cam2w[:, :3, :3] @ trans_offset.unsqueeze(-1)) # Question: new translation (the scale now is fused in camera pose estimation)
        cam2w = torch.cat((torch.cat((tmp_cam2w[:, :3, :3], new_trans), dim=2),
                          vec0001.view(1, 1, 4).expand(len(K), 1, 4)), dim=1) # Question: what is fucking this?
    ...
    K = make_K_cam_depth(log_focals, pps, None, None, None, None) # set initial intrinsics
    ...
    core_depth = [nn.Parameter(d.ravel().to(dtype)) for d in core_depth] # Question: Does core_depth is sampled features? core_depth shape: (1/8 resolution of original images)
    log_sizes = [nn.Parameter(torch.zeros(1, dtype=dtype, device=device)) for _ in range(len(imgs))] # Question: What is log_sizes?
    ...
    def loss_dust3r(cam2w, pts3d, pix_loss):
        ...
        for s in imgs_slices:
            if not is_matching_ok[s.img1, s.img2]:
                ...
                loss += tgt_confs @ pix_loss(pts3d[s.img1], tgt_pts) # in this case, not the pixel correspondence is used.
    ...
    def loss_3d(K, w2cam, pts3d, pix_loss):
        ...
        if any(v.get('freeze') for v in init.values()): # Question: does init mean we already had some pre-set values?
            pts3d_1 = []
            pts3d_2 = []
            confs = []
            for s in imgs_slices:
            ...
            pts3d_1 = [pts3d[s.img1][s.slice1] for s in imgs_slices if is_matching_ok[s.img1, s.img2]] # Briefly, we do not need full-resolution images and all matches
        ...
        if pts3d_1 != []:
            ...
            loss = confs @ pix_loss(pts3d_1, pts3d_2) # @Shuo: the same point predicted by two different images; it is the same as Dust3R implementation, aligning the 3D prediction
            cf_sum = confs.sum()
        else:
            loss = 0.
            cf_sum = 1.
    ...
    def loss_2d(K, w2cam, pts3d, pix_loss):
        # For each correspondence, we have two 3D points (one for each image of the pair).
        # For each 3D point, we have 2 reproj errors; Question: the difference between the loss_3d?
        ...
        for img1, pix1, confs, cf_sum, imgs_slices in corres2d:
            ...
            if pts3d_in_img1 != []:
                ...
                loss += confs_filtered @ pix_loss(pix1_filtered, reproj2d(proj_matrix[img1], pts3d_in_img1)) # this is the 2D-2D correspondence loss
    ...
    def optimize_loop(loss_func, lr_base, niter, pix_loss, lr_end=0):
        ...
        with tqdm(total=niter) as bar:
            for iter in range(niter or 1):
                ...
                pts3d = make_pts3d(anchors, K, cam2w, depthmaps, base_focals=base_focals) # get 3d points in the world coordinate
                if niter == 0:
                    break
                ...
                pix_loss = ploss(1 - alpha) # if using the input pix_loss, it does not work in fact.
                optimizer.zero_grad()
                loss = loss_func(K, w2cam, pts3d, pix_loss) + loss_dust3r_w * loss_dust3r(cam2w, pts3d, lossd) # loss_func: loss3d or 2d
                loss.backward()
                optimizer.step()

                # make sure the pose remains well optimizable
                for i in range(len(imgs)):
                    quats[i].data[:] /= quats[i].data.norm() # the quaternion should be normalized
    ...
    for i, img in enumerate(imgs):
        ...
        quats[i].requires_grad_(trainable) # camera pose
        trans[i].requires_grad_(trainable)
        log_sizes[i].requires_grad_(trainable) # is it the scale?
...
def make_pts3d(anchors, K, cam2w, depthmaps, base_focals=None, ret_depth=False):
    ...
    for img, (pixels, idxs, offsets) in anchors.items():
        ...
        pts3d = proj3d(invK[img], pixels, depthmaps[img][idxs] * offsets) # NOTE The basics is still on the depth map and the camera intrinsics
        if ret_depth:
...
def forward_mast3r(pairs, model, cache_path, desc_conf='desc_conf',
    ...
    for img1, img2 in tqdm(pairs): # in-fact, there is no need fot C_n^2 pairs, since the prediction will output paired result.
        ...
        if not all(os.path.isfile(p) for p in (path1, path2, path_corres)):
            ...
            X11, X21, X22, X12 = [r['pts3d'][0] for r in res] # NOTE 12 means the first iamge prediction in the 2nd image; so 11 and 21 are a piar
            ...
            corres = extract_correspondences(descs, qonfs, device=device, subsample=subsample) # get the correspondence pixels of two images and pairs' confidence

            conf_score = (C11.mean() * C12.mean() * C21.mean() * C22.mean()).sqrt().sqrt() # all image's pairs's confidence
            matching_score = (float(conf_score), float(corres[2].sum()), len(corres[2])) # first one is image-pair, the second one is selected correspondence.
...
@torch.no_grad()
def paris_symmetric_inference(pairs, model, device):
    res = {}
    for img1, img2 in tqdm(pairs):
        res[img1['instance'], img2['instance']] = symmetric_inference(model, img1, img2, device)
    return res
...
@torch.no_grad()
def paris_asymmetric_inference(pairs, model, device):
    res = {}
    for img1, img2 in tqdm(pairs):
        res[img1['instance'], img2['instance']] = asymmetric_inference(model, img1, img2, device)
    return res

def asymmetric_inference(model, img1, img2, device):
    shape1 = torch.from_numpy(img1['true_shape']).to(device, non_blocking=True)
    shape2 = torch.from_numpy(img2['true_shape']).to(device, non_blocking=True)
    img1 = img1['img'].to(device, non_blocking=True)
    img2 = img2['img'].to(device, non_blocking=True)
...
def extract_correspondences(feats, qonfs, subsample=8, device=None, ptmap_key='pred_desc'):
    ...
    for A, B, QA, QB in [(feat11, feat21, qonf11.cpu(), qonf21.cpu()),
        ...
        nn1to2 = fast_reciprocal_NNs(A, B, subsample_or_initxy1=subsample, ret_xy=False, **opt) # two arrays, where stores indices of pixel index from one to the other
        nn2to1 = fast_reciprocal_NNs(B, A, subsample_or_initxy1=subsample, ret_xy=False, **opt)

        idx1.append(np.r_[nn1to2[0], nn2to1[1]]) # row-cat correspondences (not filtering the same points)
        idx2.append(np.r_[nn1to2[1], nn2to1[0]])
        qonf1.append(QA.ravel()[idx1[-1]]) # idx1[-1] is the just added index
        qonf2.append(QB.ravel()[idx2[-1]])
    ### For the above code, we now get the indices from uniformly selecting points and then coverging to the correspondence indices.
    # merge corres from opposite pairs
    H1, W1 = feat11.shape[:2]
    H2, W2 = feat22.shape[:2]
    cat = np.concatenate

    xy1, xy2, idx = merge_corres(cat(idx1), cat(idx2), (H1, W1), (H2, W2), ret_xy=True, ret_index=True) # get all correspondence when inputting a pair of image
    corres = (xy1.copy(), xy2.copy(), np.sqrt(cat(qonf1)[idx] * cat(qonf2)[idx])) # the pair's confidence, here the returned index corresponds to the shape of xy1 and xy2
...
def prepare_canonical_data(imgs, tmp_pairs, subsample, order_imgs=False, min_conf_thr=0,
    ...
    for img in tqdm(imgs):
        ...
        # collect all pred1
        n_pairs = sum((img in pair) for pair in tmp_pairs) # there is a problem here in the pairs, is there reduantant pairs?

        ptmaps11 = None
        pixels = {}
        n = 0
        for (img1, img2), ((path1, path2), path_corres) in tmp_pairs.items(): # NOTE for each image, deal with all pairs
            score = None
            if img == img1:
            ...
            if img == img2: # load the img in the created image pairs
                X, C, X2, C2 = torch.load(path2, map_location=device) # the prediction in the pair: X22, C22, X12, C12
                score, (xy1, xy2, confs) = load_corres(path_corres, device, min_conf_thr) # xy1 is the correspondence in image1; while xy2 is in image2
                pixels[img1] = xy2, confs # svae the corresponding correspondence pixels
                if img not in preds_21:
                    preds_21[img] = {}
                preds_21[img][img1] = X2, C2 # the img1 represented in the img; `X2` represents the representation in the other coordinate

            if score is not None:
                i, j = imgs.index(img1), imgs.index(img2) # imgs is the list of all images (the parent directory)
                # score = score[0]
                # score = np.log1p(score[2])
                score = score[2] # the number of correspondence in this pair
                ...
                if ptmaps11 is None:
                    H, W = C.shape
                    ptmaps11 = torch.empty((n_pairs, H, W, 3), device=device) # n_pairs: the same image will appear in multiple pairs, which is the number here
                    confs11 = torch.empty((n_pairs, H, W), device=device)

                ptmaps11[n] = X # always represented in the identity coordinate.
                confs11[n] = C
                n += 1

        if canon is None:
            canon, canon2, cconf = canonical_view(ptmaps11, confs11, subsample, **kw) # ptmaps11 are current image prediction in the identity coordination, confs are pixel-wise confidence
            del ptmaps11
            del confs11
        ...
        if focal is None:
            focal = estimate_focal_knowing_depth(canon[None], pp, focal_mode='weiszfeld', min_focal=0.0, max_focal=np.inf)
            if cache:
                torch.save(to_cpu(((canon, canon2, cconf), focal)), mkdir_for(cache))

        # extract depth offsets with correspondences
        core_depth = canon[subsample // 2::subsample, subsample // 2::subsample, 2] # subsampled-depths
        idxs, offsets = anchor_depth_offsets(canon2, pixels, subsample=subsample) # idxs: the correspondence pixels anchorled points; offsets: the depth ratio of correspondence pixels compared to anchor points

        canonical_views[img] = (pp, (H, W), focal.view(1), core_depth, pixels, idxs, offsets) # `pixels` are saved 2d correspondences.
...
def condense_data(imgs, tmp_paths, canonical_views, dtype=torch.float32):
    ...
    set_imgs = set(imgs) # the list of all input images
    ...
    for idx1, img1 in enumerate(imgs):
        # load stuff
        pp, shape, focal, anchors, pixels_confs, idxs, offsets = canonical_views[img1] #canonical_views stores the initial data of one image
        # compared to Dust3R initialization, it seems in Mast3R initialization, the intialization is conducted by averaging the depthmap
        ...
        for img2, (pixels, match_confs) in pixels_confs.items(): # first element of `.items()` is the key, the second is the value
            if img2 not in set_imgs:
                continue
            assert len(pixels) == len(idxs[img2]) == len(offsets[img2])
            img_uv1.append(torch.cat((pixels, torch.ones_like(pixels[:, :1])), dim=-1)) # N,3
            img_idxs.append(idxs[img2])
            img_offs.append(offsets[img2]) # NOTE:is it what I understood that use the another image for correspondence?
            cur_n.append(cur_n[-1] + len(pixels))
            # store the position of 3d points
            tmp_pixels[img1, img2] = pixels.to(dtype), match_confs.to(dtype), slice(*cur_n[-2:]) # this slice specifies the range of correspondence pixels for this pair
    ...
    for img1, img2 in tmp_paths:
        try:
            pix1, confs1, slice1 = tmp_pixels[img1, img2] # here `pix1` in fact is the pixels in the second image
            pix2, confs2, slice2 = tmp_pixels[img2, img1]
        except KeyError:
            continue
        img1 = imgs.index(img1) # here, img1 and img2 are vonverted to image index
...
def canonical_view(ptmaps11, confs11, subsample, mode='avg-angle'):
    ...
    confs11 = confs11.unsqueeze(-1) - 0.999 # Question why -0.999
    canon = (confs11 * ptmaps11).sum(0) / confs11.sum(0) # sum along the first axis->weighted average

    canon_depth = ptmaps11[..., 2].unsqueeze(1)
    S = slice(subsample // 2, None, subsample)
    center_depth = canon_depth[:, :, S, S] # sample the collected depth map, note that the current sample is not identical to the correspondence indices because after iteratios, the correspondence indices have already changed
    assert (center_depth > 0).all()
    stacked_depth = F.pixel_unshuffle(canon_depth, subsample) # change the shape of the tensor: (n,1,H,W)->(n,1*subsample*subsample,H//subsample,W//subsample)
    stacked_confs = F.pixel_unshuffle(confs11[:, None, :, :, 0], subsample) #, similar, change the confs shape to stacked shape
    ...
    if mode == 'avg-reldepth':
        ...
        xy = ptmaps11[..., 0:2].permute(0, 3, 1, 2) # (N,2,H,W), NOTE! note that here the `xy` are not 2d pixel coordinate, but the first two elements of the 3d points!
        stacked_xy = F.pixel_unshuffle(xy, subsample) # stack shape
        B, _, H, W = stacked_xy.shape
        stacked_radius = (stacked_xy.view(B, 2, -1, H, W) - xy[:, :, None, S, S]).norm(dim=1) # B,2,1*subsample*subsample,H//subsample,W//subsample - B,2,1,[subsample//2:None:subsample],[subsample//2:None:subsample]
        stacked_radius.clip_(min=1e-8)

        stacked_angle = torch.arctan((stacked_depth - center_depth) / stacked_radius) # anyway, the angle is the difference between the XY-plane difference and the z-axis difference
        avg_angle = (stacked_confs * stacked_angle).sum(dim=0) / stacked_confs.sum(dim=0) # for each batch, compute the average angle for each point
        # the difference is where we make the average, on the relative depth or the angle
        # back to depth
        stacked_depth = stacked_radius.mean(dim=0) * torch.tan(avg_angle) # the absoulte depth
        # canon: the initial weighted-average point maps;
        canon2 = F.pixel_shuffle((1 + stacked_depth / canon[S, S, 2]).unsqueeze(0), subsample).squeeze() # what is it
    else:
        raise ValueError(f'bad {mode=}')

    confs = (confs11.square().sum(dim=0) / confs11.sum(dim=0)).squeeze()
    return canon, canon2, confs # anyway, `canon2` is the relative depth ratio.


def anchor_depth_offsets(canon_depth, pixels, subsample=8):
    device = canon_depth.device #canon_depth: the relative depth ratio compared to central depth
    ...
    core_depth = canon_depth[cy, cx] # note that since relative depth ratio compared to central points, the core_depth are all 1
    assert (core_depth > 0).all() # Question: here, what is the canon depth
    ...
    for img2, (xy1, _confs) in pixels.items(): # pixels consists of another image in a piar of images
        px, py = xy1.long().T # xy1 are the correspondence pixels in the image

        # find nearest anchor == block quantization
        core_idx = (py // subsample) * W2 + (px // subsample) # Question: but they start from 0.
        ...
        offset = pts_z / ref_z # offset are depth ratio of correspondence pixels compared to the anchor points; where anchor points are just suusampled ones
...
def compute_min_spanning_tree(pws):
    ...
    for i, j in pws.nonzero().cpu().tolist():
        sparse_graph[i, j] = -float(pws[i, j]) # store negative matching scores in the graph; negative means sorting from smallest to largest
    msp = sp.csgraph.minimum_spanning_tree(sparse_graph) # we already have a minimum spanning tree now

    # now reorder the oriented edges, starting from the central point
    ranks1, _ = bfs(msp, 0)
    ranks2, _ = bfs(msp, ranks1.argmax()) # the last node from the starting node `0`
    ranks1, _ = bfs(msp, ranks2.argmax())
    # this is the point farther from any leaf
    root = np.minimum(ranks1, ranks2).argmax() # average the longest distance to the two trees
    ...
    return root, edges # the root, the parent-child edge
